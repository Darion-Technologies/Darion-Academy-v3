    ...
    <div className="gradient-welcome relative flex h-full flex-col justify-between overflow-hidden rounded-xl p-3 sm:p-4 text-white shadow-[var(--shadow-md)]">
      ...
      <div className="relative z-10 mt-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div
          ...
          className={cn(
            "relative w-fit shrink-0 overflow-hidden rounded-xl border px-2.5 py-1.5 sm:px-3 sm:py-2 backdrop-blur-sm transition-all",
            hasActiveStreak
              ? "border-primary/40 bg-white/10 text-white shadow-[0_0_24px_rgba(0,140,187,0.3),inset_0_1px_0_rgba(255,255,255,0.15)]"
              : "border-white/15 bg-white/7 text-white/75",
          )}
        >
          {hasActiveStreak && (
            <>
              <div className="streak-halo pointer-events-none absolute -left-4 top-1/2 size-20 -translate-y-1/2 rounded-full bg-primary/40 blur-2xl" />
              <div className="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-primary/50 to-transparent" />
            </>
          )}
          <div className="relative flex items-center gap-1">
            {hasActiveStreak && (
              <span className="text-2xl sm:text-3xl leading-none select-none animate-pulse drop-shadow-[0_0_10px_rgba(255,100,0,0.8)]" style={{ animationDuration: '1.5s' }}>🔥</span>
            )}
            <span
              aria-hidden="true"
              className={cn(
                "text-3xl sm:text-4xl font-semibold leading-none tracking-tight",
                hasActiveStreak && "text-white drop-shadow-[0_0_16px_rgba(255,255,255,0.5)]",
              )}
            >
              {currentStreak}
            </span>
