...
export function BottomRowSkeleton() {
  return (
    <div className="grid grid-cols-1 items-stretch gap-3 xl:grid-cols-[1.8fr_1fr]">
      {/* Heatmap */}
      <div className="flex min-h-[280px] flex-col overflow-hidden rounded-xl border bg-card text-card-foreground shadow">
            ...
            <div className="flex gap-[2px] overflow-hidden">
              {[...Array(53)].map((_, i) => (
                <div key={i} className="flex flex-col gap-[2px]">
                  {[...Array(7)].map((_, j) => (
                    <Skeleton key={j} className="h-[12px] w-[12px] rounded-[2px]" />
                  ))}
                </div>
              ))}
            </div>
