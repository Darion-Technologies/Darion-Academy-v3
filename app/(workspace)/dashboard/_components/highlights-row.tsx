        ...
        <Link href="/courses" className="bg-card rounded-xl border border-border p-4 shadow-sm relative overflow-hidden block transition-all hover:scale-[1.02] active:scale-[0.98] hover:shadow-md hover:bg-muted/40 group">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-1.5 text-[10px] sm:text-xs font-semibold text-muted-foreground group-hover:text-foreground transition-colors">
              ...
              <span className="truncate">Courses Enrolled</span>
            </div>
          </div>
          <div className="flex items-end justify-between">
            <span className="text-lg sm:text-xl font-medium text-foreground tracking-tight">{stats.totalCourses < 10 ? `0${stats.totalCourses}` : stats.totalCourses}</span>
            <div className="flex items-end gap-1 h-6 sm:h-8">
              <div className="w-1 sm:w-1.5 h-2 sm:h-3 bg-primary/30 rounded-sm"></div>
              <div className="w-1 sm:w-1.5 h-3 sm:h-5 bg-primary/50 rounded-sm"></div>
              <div className="w-1 sm:w-1.5 h-2 sm:h-4 bg-primary/70 rounded-sm"></div>
              <div className="w-1 sm:w-1.5 h-6 sm:h-8 bg-primary rounded-sm"></div>
            </div>
          </div>
        </Link>
        ...
        <Link href="/progress" className="bg-card rounded-xl border border-border p-4 shadow-sm block transition-all hover:scale-[1.02] active:scale-[0.98] hover:shadow-md hover:bg-muted/40 group">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-1.5 text-[10px] sm:text-xs font-semibold text-muted-foreground group-hover:text-foreground transition-colors">
              ...
              <span className="truncate">Lessons Done</span>
            </div>
          </div>
          <div className="flex items-end justify-between">
            <span className="text-lg sm:text-xl font-medium text-foreground tracking-tight">{lessonsCompleted < 10 ? `0${lessonsCompleted}` : lessonsCompleted}</span>
            <div className="w-12 sm:w-16 h-6 sm:h-8 text-primary">
        ...
        <Link href="/progress" className="bg-card rounded-xl border border-border p-4 shadow-sm block transition-all hover:scale-[1.02] active:scale-[0.98] hover:shadow-md hover:bg-muted/40 group">
          ...
          <div className="flex items-end justify-between">
            <div className="flex items-baseline">
              <span className="text-lg sm:text-xl font-medium text-foreground tracking-tight">{stats.avgQuizScore}</span>
              <span className="text-sm sm:text-base font-medium text-foreground">%</span>
            </div>
            <div className="w-10 sm:w-12 h-6 sm:h-8 text-primary relative">
        ...
        <Link href="/leaderboard" className="bg-card rounded-xl border border-border p-4 shadow-sm block transition-all hover:scale-[1.02] active:scale-[0.98] hover:shadow-md hover:bg-muted/40 group">
          ...
          <div className="flex items-end justify-between">
            <div className="flex items-baseline gap-1">
              <span className="text-lg sm:text-xl font-medium text-foreground tracking-tight">{stats.currentStreak < 10 ? `0${stats.currentStreak}` : stats.currentStreak}</span>
              <span className="text-[10px] sm:text-xs font-medium text-muted-foreground mb-0.5">Days</span>
            </div>
            <div className="flex gap-1 sm:gap-1.5 mb-1.5">
              {[...Array(4)].map((_, i) => (
                <div key={i} className={cn("size-2.5 sm:size-3.5 rounded-full relative flex items-center justify-center", i < Math.min(stats.currentStreak, 4) ? "bg-primary" : "border border-primary/30")}>
                  {i < Math.min(stats.currentStreak, 4) && <div className="size-1 sm:size-1.5 bg-white rounded-full"></div>}
                </div>
              ))}
            </div>
          </div>
